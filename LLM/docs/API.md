...
| `chatroom_id` | string | ❌ | `chatroom.chatroom_id` (UUID). **답변 생성에는 안 쓰입니다.** 아래 설명 참고 |
| `history` | array | ❌ | 이전 대화. **1차에서는 생략**하고, 후속 질문 지원이 필요해지면 그때 채워 보내면 됩니다 (아래 설명) |

**반드시 보내야 하는 건 `message` 하나뿐입니다.**

`chatroom_id` 는 **서버 로그에만 남습니다.** 이 서비스는 대화를 저장하지 않아서 방을 구분할 이유가 없습니다.
다만 시연 중에 "이 대화방에서 이상한 답이 나왔다"는 제보가 오면 로그에서 그 대화만 추려낼 수 있으니,
이미 값을 갖고 계시면 같이 보내주시는 편이 디버깅에 도움이 됩니다. 안 보내도 동작은 동일합니다.
