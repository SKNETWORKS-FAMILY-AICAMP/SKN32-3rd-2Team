...
> 자동 생성: `python bench/report.py` · 2026-08-03 15:06
> 원본 측정값은 `bench/results/*.jsonl` (git 미포함), 질문 세트는 `bench/questions.yaml`.

## 1. 측정 방법과 그 한계

`bench/questions.yaml` 의 **34문항**을 실제 서비스 경로(`generate_answer`)로
...
걸려 34문항 한 조건에 17분이 든다 — 프롬프트를 고쳐가며 비교하는 루프를
돌릴 수 없다.
...
| **실제 RAG** | **32/34** | **28/32 (88%)** | **3.9s** | **5.7s** | **0건** |

검색이 실제로 붙으면서 **지연이 2.0초 늘고
검색 recall 이 12%p 떨어졌다.** 주제 정확도는 거의 영향이 없다
— 주제 분류는 질문 의도로 판단하고 문서명은 힌트로만 쓰기 때문이다.

p95 가 5.7초로 스토리보드(13p)의 5초 기준에 근접한다. 스트리밍
...
|---|---|---|---|
| `leave-annual-a` | 연차 며칠까지 쓸 수 있나요? | 복무규정.pdf, 5.근로기준법(법률).pdf | 3.교육공무원법(법률).pdf, 4.교육공무원임용령(시행령).pdf, 5.근로기준법(시행령).pdf |
| `work-duty-general` | 직원 복무 관련 기본 규정이 어떻게 되나요? | 복무규정.pdf | 시간선택제 직원에 관한 규칙.pdf, 직원인사규정 시행규칙.pdf, 직원인사규정.pdf, 휴직자 복무관리 지침.pdf |
| `disc-code-of-conduct` | 교직원 행동강령에서 금지하는 행위는 무엇인가요? | 한국기술교육대학교 교직원행동강령.pdf | 2.공공기관의 운영에 관한 법률(시행령).pdf, 3.교육공무원법(법률).pdf, 교직원 음주운전 비위행위 확인에 관한 지침.pdf, 복무규정.pdf |
| `disc-crime-report` | 직무 관련 범죄를 알게 되면 어떻게 신고하나요? | 한국기술교육대학교 직무범죄고발지침.pdf | 1.남녀고용평등과 일ㆍ가정 양립 지원에 관한 법률(법률).pdf, 1.남녀고용평등과 일ㆍ가정 양립 지원에 관한 법률(시행령).pdf, 2.공공기관의 운영에 관한 법률(법률).pdf, 2.공공기관의 운영에 관한 법률(시행령).pdf |
...
문제가 아니라 데이터 누락이다. 이 둘을 넣으면 recall 은 30/32
(94%)가 된다.
