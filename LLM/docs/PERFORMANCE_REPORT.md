...
> 자동 생성: `python bench/report.py` · 2026-08-03 17:17
...
| **실제 RAG** | **32/34** | **26/32 (81%)** | **3.6s** | **4.7s** | **0건** |

검색이 실제로 붙으면서 **지연이 1.7초 늘고
검색 recall 이 19%p 떨어졌다.** 주제 정확도는 거의 영향이 없다
— 주제 분류는 질문 의도로 판단하고 문서명은 힌트로만 쓰기 때문이다.

p95 가 4.7초로 스토리보드(13p)의 5초 기준에 근접한다. 스트리밍
...
|---|---|---|---|
| `leave-annual-a` | 연차 며칠까지 쓸 수 있나요? | 복무규정.pdf, 5.근로기준법(법률).pdf |  |
...
| `oos-personal-balance` | 제 연차가 지금 며칠 남았는지 알려주세요 | 복무규정.pdf, 5.근로기준법(법률).pdf |  |
...
문제가 아니라 데이터 누락이다. 이 둘을 넣으면 recall 은 28/32
(88%)가 된다.
